# RBRTW / EWX Real Maps 2.2.5

This is the Real Maps V2.2.5 cleanup and feature-preservation build.


## Major fixes in 2.2.5

- Added ArcGIS Basemap Styles support for MapLibre using a visible browser-side token/API key.
- Added ArcGIS Navigation, Streets, Outdoor, Topographic, Light Gray, Dark Gray, Navigation Night, Open Streets, and Open Navigation options.
- OpenFreeMap basemaps remain as fallback options.
- Road label handling now separates regular road names from highway shield / route identifier layers.
- Added Show highway shields / route identifiers and Highway shield size controls.
- Added Broadcast Max road-label density to make road names and route identifiers more aggressive for graphic export.
- Existing NOAA/MRMS/SPC/WPC/NDFD/OpenWeather layers, custom city values, editable keys, county overlays, text boxes, symbols, and PNG export are preserved.

ArcGIS basemaps use the ArcGIS Basemap Styles service. Paste your ArcGIS token into the Basemap panel and click **Save ArcGIS Token In Browser**. The token is stored in localStorage for that browser.

## Preserved 2.2.4 fixes
