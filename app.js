...
function bindEvents(){
  ...
  ['cityPreset','cityDensity','cityValueSize','cityNameSize','cityTextColor','cityHaloColor'].forEach(id => $(id)?.addEventListener('input', drawCityLabels));
...
function getSelectedCities(){
  ...
  if (preset === 'custom') return parseCityLines();
  const list = CITY_SETS[preset] || CITY_SETS.texasCore || [];
  return list.filter(c => c.p <= density).map(c => ({...c, value: cityValueMap.get(c.name) || ''}));
}
...
function drawCityLabels(){
  ...
  for (const c of cities) {
    ...
    const displayValue = c.value || cityValueMap.get(c.name) || '';
...
async function loadPointValues(){
  const preset = $('cityPreset')?.value || 'custom';
  const cities = visibleCities().slice(0, 70);
  setLayerStatus(`Loading NWS point temperatures for ${cities.length} visible city labels...`);
  const updatedLines = [];
  for (const c of cities) {
    try {
      const value = await fetchCityDisplayValue(c);
      cityValueMap.set(c.name, value);
      updatedLines.push(`${c.name}|${c.lat}|${c.lon}|${value}`);
    } catch (err) {
      cityValueMap.set(c.name, c.value || '');
      updatedLines.push(`${c.name}|${c.lat}|${c.lon}|${c.value || ''}`);
    }
  }
  if (preset === 'custom') $('cityValues').value = updatedLines.join('\n');
  pointValuesLoaded = true;
  drawCityLabels();
  setLayerStatus(`Loaded NWS point values for ${cities.length} visible cities. Increase density slowly to avoid throttling.`);
}


async function fetchCityDisplayValue(c){
...
function drawCitiesToCanvas(ctx){
  ...
  for (const c of getSelectedCities()){
    ...
    cityShadow(ctx, halo); const displayValue = c.value || cityValueMap.get(c.name) || '';
