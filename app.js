...
async function initMap(){
  ...
  let initialStyle = OPENFREE_BASEMAPS.liberty;
  try {
    initialStyle = await resolveSelectedBasemapStyle();
  } catch (err) {
    console.error(err);
    setLayerStatus(`ArcGIS basemap failed on startup: ${err.message}. Using OpenFreeMap fallback.`);
    initialStyle = OPENFREE_BASEMAPS.liberty;
  }
...
async function resolveSelectedBasemapStyle(){
  ...
  if (provider === 'arcgisvt') return await loadArcgisVectorTileServerStyle(styleId);
...
function drawFooter(ctx){ ctx.fillStyle='rgba(2,7,15,.94)'; ctx.fillRect(0,1036,1920,44); ctx.fillStyle='rgba(255,255,255,.76)'; ctx.fillRect(0,1036,1920,4); text(ctx,'Generated by RBRTW / EWX Real Maps 2.2.7',34,1064,16,'left','#d5e4ff',900); text(ctx,'Layer stack export',1886,1064,16,'right','#a9c0e2',900); }
