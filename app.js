...
const ARCGIS_TOKEN_STORAGE_KEY = 'rbrtw_arcgis_basemap_token';
...
const TEXAS_COUNTIES_GEOJSON_URL = "/.netlify/functions/noaa-map?op=counties";
...
const PRODUCTS = {
  radar: {
    title: 'CLOUDS + RADAR SNAPSHOT', subtitle:'', service:'radar', layers:'', sourceLabel:'NOAA MRMS radar base reflectivity MapServer',
...
function bindEvents(){
  ...
  $('saveArcgisToken')?.addEventListener('click', saveArcgisToken);
  $('arcgisToken')?.addEventListener('change', () => { saveArcgisToken(false); if (($('basemapStyle')?.value || '').startsWith('arcgis:')) setBasemap(); });
...
function initMap(){
  ...
  map = new maplibregl.Map({
    container: 'map',
    style: OPENFREE_BASEMAPS.liberty,
...
async function setBasemap(){
  ...
  try {
    style = show ? await resolveSelectedBasemapStyle() : BLANK_STYLE;
  } catch (err) {
    console.error(err);
...
async function resolveSelectedBasemapStyle(){
  ...
  if (provider === 'arcgis') return await loadArcgisBasemapStyle(styleId);
...
async function loadArcgisBasemapStyle(styleId){
  ...
  if (!token) throw new Error('ArcGIS basemap token is missing. Paste the token in the Basemap panel, save it, then reload the ArcGIS basemap.');
  ...
  return patchArcgisStyleToken(style, token);
}

function patchArcgisStyleToken(style, token){
  ...
  const addToken = (url) => {
    ...
    if (!/arcgis\.com|arcgisonline\.com|tiles\.arcgis\.com|basemapstyles-api\.arcgis\.com/i.test(url)) return url;
...
function getArcgisToken(){
  return ($('arcgisToken')?.value || localStorage.getItem(ARCGIS_TOKEN_STORAGE_KEY) || '').trim();
}

function loadSavedArcgisToken(){
  const input = $('arcgisToken');
  if (!input) return;
  input.value = localStorage.getItem(ARCGIS_TOKEN_STORAGE_KEY) || '';
}

function saveArcgisToken(showMessage=true){
  const token = ($('arcgisToken')?.value || '').trim();
  if (token) localStorage.setItem(ARCGIS_TOKEN_STORAGE_KEY, token);
  else localStorage.removeItem(ARCGIS_TOKEN_STORAGE_KEY);
  if (showMessage) setLayerStatus(token ? 'ArcGIS token saved in this browser.' : 'ArcGIS token cleared from this browser.');
}
...
function addTexasCountyLayers(){
  ...
  try {
    if (!map.getSource('texas-counties-source')) {
      map.addSource('texas-counties-source', {type:'geojson', data:TEXAS_COUNTIES_GEOJSON_URL});
      countiesLoaded = true;
    }
    if (!map.getLayer('texas-counties-line')) {
      map.addLayer({
        id:'texas-counties-line', type:'line', source:'texas-counties-source',
        paint:{'line-color':$('countyLineColor')?.value || '#75e6ff','line-width':Number($('countyLineWidth')?.value || 2.2),'line-opacity':0.86}
      });
    }
    if (!map.getLayer('texas-counties-label')) {
      map.addLayer({
        id:'texas-counties-label', type:'symbol', source:'texas-counties-source',
        layout:{'text-field':['coalesce',['get','BASENAME'],['get','NAME']],'text-size':Number($('countyNameSize')?.value || 14),'text-font':['Noto Sans Bold'],'text-allow-overlap':false,'text-ignore-placement':false},
        paint:{'text-color':$('countyNameColor')?.value || '#d7f7ff','text-halo-color':$('countyNameHaloColor')?.value || '#03101f','text-halo-width':2.2,'text-opacity':0.9}
      });
    }
...
async function exportPng(){
  ...
  try {
    ...
    await downloadCanvas(canvas, `RBRTW-real-map-2-2-4-${slug(PRODUCTS[currentProductKey].title)}-${new Date().toISOString().slice(0,10)}.png`);
...
function drawFooter(ctx){ ctx.fillStyle='rgba(2,7,15,.94)'; ctx.fillRect(0,1036,1920,44); ctx.fillStyle='rgba(255,255,255,.76)'; ctx.fillRect(0,1036,1920,4); text(ctx,'Generated by RBRTW / EWX Real Maps 2.2.5',34,1064,16,'left','#d5e4ff',900); text(ctx,'Layer stack export',1886,1064,16,'right','#a9c0e2',900); }
