...
function bindEvents(){
  ...
  ['showRoadLayers','showRoadLabels','roadLineColor','highwayLineColor','roadLabelColor','roadLabelHaloColor','roadLabelSize','roadLabelDensity','roadLineWidth','highwayLineWidth'].forEach(id => $(id)?.addEventListener('input', applyBasemapRoadStyling));
...
function applyBasemapRoadStyling(){
  ...
  const expandedLabels = ($('roadLabelDensity')?.value || 'expanded') === 'expanded';
  const style = map.getStyle();
  ...
  for (const id of basemapControlGroups.roadLines) {
    ...
    try {
      ...
      if (showRoads) {
        ...
        map.setPaintProperty(id, 'line-opacity', highway ? 0.95 : 0.78);
      }
    } catch (err) {}
  }
    ...
    try {
      ...
      if (showRoadLabels) {
        ...
        if (expandedLabels) {
          ...
          map.setLayoutProperty(id, 'symbol-z-order', 'viewport-y');
        } else {
...
function drawFooter(ctx){ ctx.fillStyle='rgba(2,7,15,.94)'; ctx.fillRect(0,1036,1920,44); ctx.fillStyle='rgba(255,255,255,.76)'; ctx.fillRect(0,1036,1920,4); text(ctx,'Generated by RBRTW / EWX Real Maps 2.2.4',34,1064,16,'left','#d5e4ff',900); text(ctx,'Layer stack export',1886,1064,16,'right','#a9c0e2',900); }
