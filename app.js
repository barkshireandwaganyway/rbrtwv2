...
let basemapControlGroups = {placeLabels:[], roadLines:[], roadLabels:[]};
...
function bindEvents(){
  ...
  ['showRoadLayers','showRoadLabels','roadLineColor','highwayLineColor','roadLabelColor','roadLabelHaloColor'].forEach(id => $(id)?.addEventListener('input', applyBasemapRoadStyling));
  ['showStations','stationDisplayMode','stationDensity','stationValueSize','stationNameSize','stationTextColor','stationHaloColor','stationDotColor','stationDetailMode'].forEach(id => $(id)?.addEventListener('input', drawStations));
  $('stationSelect')?.addEventListener('change', () => { updateSelectedStationsFromControl(); drawStations(); });
  $('loadStations')?.addEventListener('click', loadNwsStationsInView);
  $('selectAllStations')?.addEventListener('click', () => { selectedStationIds = new Set(stationObservations.map(s => s.id)); renderStationSelector(); drawStations(); });
  $('clearStationSelection')?.addEventListener('click', () => { selectedStationIds.clear(); renderStationSelector(); drawStations(); });
  $('clearStations')?.addEventListener('click', () => { stationObservations=[]; selectedStationIds.clear(); stationValueMap.clear(); renderStationSelector(); drawStations(); setLayerStatus('Station observations cleared.'); });
...
function classifyBasemapLayers(){
  basemapControlGroups = {placeLabels:[], roadLines:[], roadLabels:[]};
...
function ensureBasemapLayerGroups(){
  if (!basemapControlGroups.placeLabels.length && !basemapControlGroups.roadLines.length && !basemapControlGroups.roadLabels.length) {
    classifyBasemapLayers();
  }
}
...
function applyBasemapRoadStyling(){
  ...
  for (const id of basemapControlGroups.roadLines) {
    ...
    try {
      ...
      if (showRoads) {
        const color = isHighwayLayer(layer) ? highwayColor : roadColor;
...
async function loadNwsStationsInView(){
  ...
  try {
    ...
    selectedStationIds = new Set(loaded.slice(0, Math.min(8, loaded.length)).map(s => s.id));
    $('showStations').checked = true;
    if ($('stationDisplayMode')) $('stationDisplayMode').value = 'selected';
    renderStationSelector();
    drawStations();
    setLayerStatus(`Loaded ${loaded.length} NWS station observations. Pick stations in the dropdown/list to show exactly the ones you want.`);
  } catch (err) {
    console.error(err);
...
function updateSelectedStationsFromControl(){
  const sel = $('stationSelect');
  if (!sel) return;
  selectedStationIds = new Set([...sel.selectedOptions].map(o => o.value));
}

function renderStationSelector(){
  const sel = $('stationSelect');
  if (!sel) return;
  sel.innerHTML = stationObservations.map(st => {
    const label = `${st.id || ''}${st.name ? ' — ' + st.name : ''}${st.value ? ' — ' + st.value : ''}`;
    return `<option value="${escapeHtml(st.id)}" ${selectedStationIds.has(st.id) ? 'selected' : ''}>${escapeHtml(label)}</option>`;
  }).join('');
}

function selectedStationsForDisplay(){
  const mode = $('stationDisplayMode')?.value || 'selected';
  if (mode === 'all') return stationObservations;
  return stationObservations.filter(st => selectedStationIds.has(st.id));
}
...
function drawStations(){
  ...
  for (const st of selectedStationsForDisplay()) {
    const p = map.project([st.lon, st.lat]);
    if (p.x < -70 || p.y < -70 || p.x > $('mapStage').clientWidth+70 || p.y > $('mapStage').clientHeight+70) continue;
    const div = document.createElement('div');
    div.className = 'stationLabel';
    div.style.left = `${p.x}px`; div.style.top = `${p.y}px`;
    const showDetail = $('stationDetailMode')?.value === 'on';
    div.innerHTML = `<div class="stationDot"></div>${st.value ? `<div class="stationValue">${escapeHtml(st.value)}</div>` : ''}<div class="stationName">${escapeHtml(st.id || st.name)}</div>${showDetail && st.detail ? `<div class="stationDetail">${escapeHtml(st.detail)}</div>` : ''}`;
    layer.appendChild(div);
  }
}
...
function applyStationStyle(){
  const layer = $('stationLayer');
  if (!layer) return;
  layer.style.setProperty('--station-text-color', $('stationTextColor')?.value || '#ffffff');
  layer.style.setProperty('--station-halo-color', $('stationHaloColor')?.value || '#000000');
  layer.style.setProperty('--station-value-size', `${$('stationValueSize')?.value || 34}px`);
  layer.style.setProperty('--station-name-size', `${$('stationNameSize')?.value || 14}px`);
  layer.style.setProperty('--station-dot-color', $('stationDotColor')?.value || '#7efcff');
}
...
async function exportPng(){
  ...
  try {
    ...
    await downloadCanvas(canvas, `RBRTW-real-map-2-2-3-${slug(PRODUCTS[currentProductKey].title)}-${new Date().toISOString().slice(0,10)}.png`);
...
async function manualExportCanvas(){
  ...
  try {
    ...
    ctx.restore();
    throw new Error(`Map canvas could not be exported. A basemap/weather tile probably blocked canvas export: ${err.message}`);
  }
  ctx.restore();
  drawMapFrame(ctx);
...
function drawFooter(ctx){ ctx.fillStyle='rgba(2,7,15,.94)'; ctx.fillRect(0,1036,1920,44); ctx.fillStyle='rgba(255,255,255,.76)'; ctx.fillRect(0,1036,1920,4); text(ctx,'Generated by RBRTW / EWX Real Maps 2.2.3',34,1064,16,'left','#d5e4ff',900); text(ctx,'Layer stack export',1886,1064,16,'right','#a9c0e2',900); }
...
function drawStationsToCanvas(ctx){
  const color = $('stationTextColor')?.value || '#ffffff';
  const halo = $('stationHaloColor')?.value || '#000000';
  const dotColor = $('stationDotColor')?.value || '#7efcff';
  const valueSize = Number($('stationValueSize')?.value || 34);
  const nameSize = Number($('stationNameSize')?.value || 14);
  const showDetail = $('stationDetailMode')?.value === 'on';
  for (const st of selectedStationsForDisplay()){
    const p = map.project([st.lon,st.lat]); const x=26+p.x, y=134+p.y; if(x<0||x>1920||y<120||y>1036) continue;
    ctx.save();
    ctx.fillStyle = dotColor; ctx.strokeStyle = 'rgba(0,0,0,.85)'; ctx.lineWidth = 3;
    ...
    if(showDetail && st.detail) text(ctx,st.detail,x,y+Math.round(nameSize*3.0),Math.max(9, nameSize-1),'center',color,1000);
